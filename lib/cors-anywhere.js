...
function getHandler(options, proxy) {
  ...
  return function (req, res) {
    ...
    if (!location) {
      // Check if the request was specifically for the root path '/'
      if (req.url === '/') {
          // Return 200 OK with a funny message to satisfy health checks
          console.log('[cors-anywhere] Root path access detected, returning 200 OK.'); // Optional log
          cors_headers['Content-Type'] = 'text/plain'; // Set content type
          res.writeHead(200, 'OK', cors_headers); // <--- Changed to 200 OK
          res.end('What are you doing here? Shoo!'); // <--- Funny message
          return; // Stop processing here
      }

      // Handle the specific case of missing slashes after http:/https:
      ...
      // For any other reason location is null, it's an invalid URL. Return 400 Bad Request.
      console.log(`[cors-anywhere] Invalid URL detected: ${req.url.slice(1)}, returning 400.`); // Optional log
      cors_headers['Content-Type'] = 'text/plain'; // Set content type
      res.writeHead(400, 'Bad Request', cors_headers);
      res.end('Invalid URL specified');
      return;
    }
